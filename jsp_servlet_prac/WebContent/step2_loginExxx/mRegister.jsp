	...
	$(function(){ //제이쿼리
		...
		$("#confirmPasswd").blur(function(){
			...
			if($("#passwd").val() == $("#confirmPasswd").val() ){
				...
				$("#msg").html("<span style='color:green;'> 비밀번호가 일치합니다. </span>");
				isValidConfirmPasswd = true;
			}
			
			else{
				$("#msg").html("<span style='color:red;'>비밀번호가 불일치합니다.</span>")
		...
		<form action="registerMember" method="post" enctype="multipart/form-data" onsubmit="return checkFormData();">
			...
			<table border="1">
				<tr>
					<td>아이디</td>
					<td>
					<input type="text" id="memberId" name="memberId" placeholder="아이디를 입력해주세요." required="required">
				...
				<tr>
					<td>비밀번호</td>
					<td><input type="password" id="passwd" name="passwd" placeholder="비밀번호를 입력해주세요." required="required"></td>
				</tr>
				<tr>
					<td>비밀번호 확인</td>
				...
				<tr>
					<td>성별</td>
					<td>
						남 &nbsp;<input type="radio" name="sex" value="m" checked="checked"> &emsp;
				...
				<tr>
					<td>연락처</td>
					<td>
						<input type="text" name="hp" placeholder="-를 포함해서 연락처를 입력해주세요." required="required"><br>
						SMS 소식을 수신합니다.<input type="checkbox" id="smsstsYn" name="smsstsYn" value="Y" checked="checked">
					</td>
